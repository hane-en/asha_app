...
try {
    // البحث عن المستخدم (مزود خدمة أو admin)
    $stmt = $pdo->prepare("SELECT id, name, email, phone, password, user_type, is_verified, is_active FROM users WHERE email = ? AND (user_type = 'provider' OR user_type = 'admin')");
    $stmt->execute([$email]);
    $user = $stmt->fetch();
    ...
    if (!$user['is_verified']) {
        jsonResponse(['error' => 'الحساب لم يتم التحقق منه بعد'], 403);
    }
