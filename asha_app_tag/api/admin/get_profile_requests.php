...
try {
    ...
    // جلب طلبات تحديث البيانات الشخصية
    $query = "
        SELECT 
            pr.id,
            pr.user_id,
            ...
            pr.created_at,
            pr.updated_at,
            COALESCE(u.name, 'غير محدد') as current_name,
            COALESCE(u.email, 'غير محدد') as current_email,
            COALESCE(u.phone, 'غير محدد') as current_phone
    ...
    echo json_encode([
        'success' => true,
        'message' => 'تم جلب طلبات تحديث البيانات بنجاح',
        'data' => $requests,
        'stats' => $stats
