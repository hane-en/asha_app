...
try {
    ...
    // استعلام محسن لجلب المزودين حسب الفئة - بدون profile_image
    $sql = "SELECT DISTINCT
                u.id,
                u.name,
                u.email,
    ...
    foreach ($providers as &$provider) {
        ...
        if ($provider['avg_price'] == 0) {
            $provider['avg_price'] = 100.0; // سعر افتراضي
        }
    ...
    $response = [
        ...
        'data' => [
            ...
            'category_stats' => [
                'total_services' => (int)$categoryStats['total_services'],
                'total_providers' => (int)$categoryStats['total_providers'],
                'avg_rating' => (float)$categoryStats['avg_category_rating'],
                'price_range' => [
                    'min' => (float)$categoryStats['min_category_price'],
                    'max' => (float)$categoryStats['max_category_price'],
                    'avg' => (float)$categoryStats['avg_category_price']
                ],
                'total_bookings' => (int)$categoryStats['total_category_bookings']
            ]
        ],
